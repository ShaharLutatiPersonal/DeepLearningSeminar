...
def train_and_test(mode,batch_size,epoch,data_path,verbose):
	print('**************************************************************************************************************')
	print('executing modes: ' + mode +',batch size:{} '.format(batch_size) + ',epoch:{} '.format(epoch) + ',data path = ' + data_path)
	print('**************************************************************************************************************')
	train_dataset = mnist.MNIST(data_path, train=True,download = False, transform=ToTensor())
	test_dataset = mnist.MNIST(data_path, train=False,download = False, transform=ToTensor())
	...
	res_vs_name = {'original':orig_res,'dropout':dropout_res,'batch normalization':bn_res,'weight decay':weight_decay_res}
	train_orig,train_drop,train_bn,train_wd = [],[],[],[]
	train_res_vs_name = {'original':train_orig,'dropout':train_drop,'batch normalization':train_bn,'weight decay':train_wd}
	...
	for name,model in zip(models_name,models):
		...
		for _epoch in range(epoch):
			...
			for idx, (train_x, train_label) in enumerate(train_loader):
				...
				_error = cross_error(predict_y, train_label.long()) # taking cross entropy
				sum_of_errors_in_epoch += _error.item()*train_label.shape[0] #expalnation below:
				_error.backward()
				sgd.step()
				precentile_done = round(100*idx/num_of_calls)
				if verbose:
					print('\r ['+'%s' % ("#")*precentile_done+']' + 'progress in epoch trainig {}%'.format(precentile_done) ,end = '')
			...
			if verbose:
				print()
			train_res_vs_name[name].append(1-(sum_of_errors_in_epoch/num_of_samples_train_data)/100)
			correct = 0
			sumv = 0
			...
			model.train(False) # Important Change to eval mode for Dropout to clear out
			for idx, (test_x, test_label) in enumerate(test_loader):
				...
				sumv += _est.shape[0]
			print('accuracy: {:.2f}'.format(correct / sumv) + ' epoch : {}'.format(_epoch+1))
			end_time = time.time()
...
if __name__ == '__main__':
	...
	parser.add_argument('-m', type=str,help='mode to train - str , supported : all/original/dropout/batch normalization/weight decay , default = all')
	parser.add_argument('-b', type=int,help='batch size - integer , default = 256')
	parser.add_argument('-e', type=int,help='num of epochs - integer , default = 15')
	parser.add_argument('-p', type=str,help='path to database - str , default = ./data/mnist')
	parser.add_argument('-v',type = int, help = 'verbose indicator 1 - true,default = 1')
