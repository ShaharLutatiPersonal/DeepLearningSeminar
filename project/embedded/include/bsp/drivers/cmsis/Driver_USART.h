...
#define ARM_USART_MODE_SYNCHRONOUS_MASTER                                                                              \
    (0x02UL << ARM_USART_CONTROL_Pos)  ///< Synchronous Master (generates clock signal); arg = Baudrate
#define ARM_USART_MODE_SYNCHRONOUS_SLAVE                                                                               \
    (0x03UL << ARM_USART_CONTROL_Pos)  ///< Synchronous Slave (external clock signal)
#define ARM_USART_MODE_SINGLE_WIRE                                                                                     \
    (0x04UL << ARM_USART_CONTROL_Pos)                                ///< UART Single-wire (half-duplex); arg = Baudrate
...
#define ARM_USART_SET_DEFAULT_TX_VALUE                                                                                 \
    (0x10UL << ARM_USART_CONTROL_Pos)  ///< Set default Transmit value (Synchronous Receive only); arg = value
...
#define ARM_USART_SET_SMART_CARD_GUARD_TIME                                                                            \
    (0x12UL << ARM_USART_CONTROL_Pos)  ///< Set Smart Card Guard Time; arg = number of bit periods
#define ARM_USART_SET_SMART_CARD_CLOCK                                                                                 \
    (0x13UL << ARM_USART_CONTROL_Pos)  ///< Set Smart Card Clock in Hz; arg: 0=Clock not generated
#define ARM_USART_CONTROL_SMART_CARD_NACK                                                                              \
    (0x14UL << ARM_USART_CONTROL_Pos)  ///< Smart Card NACK generation; arg: 0=disabled, 1=enabled
...
#define ARM_USART_CONTROL_BREAK                                                                                        \
    (0x17UL << ARM_USART_CONTROL_Pos)  ///< Continuous Break transmission; arg: 0=disabled, 1=enabled
...
typedef enum _ARM_USART_DRIVER_STATE {
    USART_DRIVER_STATE_INITIALIZED,
    USART_DRIVER_STATE_UNINITIALIZED,
    USART_DRIVER_STATE_POWERED,
} ARM_USART_DRIVER_STATE;
...
typedef struct _ARM_USART_CONF {
    uint32_t baud_rate;
    uint8_t  data_bits;
    uint8_t  parity;
    uint8_t  stop_bits;
...
typedef struct _ARM_DRIVER_USART {
    ...
    int32_t (*Receive)(
        void *data, uint32_t num);  ///< Pointer to \ref ARM_USART_Receive : Start receiving data from USART receiver.
    int32_t (*Transfer)(
    ...
    int32_t (*Control)(uint32_t control,
                       uint32_t arg);     ///< Pointer to \ref ARM_USART_Control : Control USART Interface.
    ARM_USART_STATUS (*GetStatus)(void);  ///< Pointer to \ref ARM_USART_GetStatus : Get USART status.
    int32_t (*SetModemControl)(ARM_USART_MODEM_CONTROL control);  ///< Pointer to \ref ARM_USART_SetModemControl : Set
                                                                  ///< USART Modem Control line state.
