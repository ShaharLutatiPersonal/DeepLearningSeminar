...
#define ARM_SAI_MASK_SLOTS_TX                                                                                          \
    (0x05U)  ///< Mask Transmitter slots; arg1 = mask (bit: 0=active, 1=inactive); all configured slots are active by
             ///< default
#define ARM_SAI_MASK_SLOTS_RX                                                                                          \
    (0x06U)  ///< Mask Receiver    slots; arg1 = mask (bit: 0=active, 1=inactive); all configured slots are active by
             ///< default
...
#define ARM_SAI_LSB_FIRST                                                                                              \
    (1U << ARM_SAI_BIT_ORDER_Pos)  ///< Data is transferred with LSB first; User Protocol only (ignored otherwise)
...
#define ARM_SAI_CLOCK_POLARITY_0                                                                                       \
    (0U << ARM_SAI_CLOCK_POLARITY_Pos)  ///< Drive on falling edge, Capture on rising  edge (default)
...
#define ARM_SAI_FRAME_LENGTH(n)                                                                                        \
    ((((n)-1) & 0x3FFU)                                                                                                \
     << ARM_SAI_FRAME_LENGTH_Pos)  ///< Frame length in bits (8..1024); default depends on protocol and data
...
#define ARM_SAI_FRAME_SYNC_WIDTH(n)                                                                                    \
    ((((n)-1) & 0xFFU) << ARM_SAI_FRAME_SYNC_WIDTH_Pos)  ///< Frame Sync width in bits (1..256); default=1; User
                                                         ///< Protocol only (ignored otherwise)
...
#define ARM_SAI_FRAME_SYNC_POLARITY_HIGH                                                                               \
    (0U << ARM_SAI_FRAME_SYNC_POLARITY_Pos)  ///< Frame Sync is active high (default); User Protocol only (ignored
                                             ///< otherwise)
#define ARM_SAI_FRAME_SYNC_POLARITY_LOW                                                                                \
    (1U << ARM_SAI_FRAME_SYNC_POLARITY_Pos)  ///< Frame Sync is active low; User Protocol only (ignored otherwise)

/*----- SAI Configuration (arg1): Frame Sync Early -----*/
#define ARM_SAI_FRAME_SYNC_EARLY                                                                                       \
    (1U << 19)  ///< Frame Sync one bit before the first bit of the frame; User Protocol only (ignored otherwise)
...
#define ARM_SAI_SLOT_COUNT(n)                                                                                          \
    ((((n)-1) & 0x1FU) << ARM_SAI_SLOT_COUNT_Pos)  ///< Number of slots in frame (1..32); default=1; User Protocol only
                                                   ///< (ignored otherwise)
...
#define ARM_SAI_SLOT_SIZE_16                                                                                           \
    (1U << ARM_SAI_SLOT_SIZE_Pos)  ///< Slot size = 16 bits; User Protocol only (ignored otherwise)
#define ARM_SAI_SLOT_SIZE_32                                                                                           \
    (3U << ARM_SAI_SLOT_SIZE_Pos)  ///< Slot size = 32 bits; User Protocol only (ignored otherwise)
...
#define ARM_SAI_SLOT_OFFSET(n)                                                                                         \
    (((n)&0x1FU) << ARM_SAI_SLOT_OFFSET_Pos)  ///< Offset of first data bit in slot (0..31); default=0; User Protocol
...
#define ARM_SAI_MCLK_PRESCALER(n)                                                                                      \
    ((((n)-1) & 0xFFFU)                                                                                                \
     << ARM_SAI_MCLK_PRESCALER_Pos)  ///< MCLK prescaler; Audio_frequency = MCLK/n; n = 1..4096 (default=1)
...
#define ARM_SAI_ERROR_FRAME_SYNC_POLARITY                                                                              \
    (ARM_DRIVER_ERROR_SPECIFIC - 13)  ///< Specified Frame Sync polarity not supported
...
typedef enum _ARM_SAI_DRIVER_STATE {
    SAI_DRIVER_STATE_INITIALIZED,
    SAI_DRIVER_STATE_UNINITIALIZED,
    SAI_DRIVER_STATE_POWERED,
} ARM_SAI_DRIVER_STATE;

typedef struct _ARM_SAI_CONF {
    uint32_t baudrate;
    uint8_t  databits;
    uint8_t  word_size;
    bool     is_mono;
