...
int32_t i2s_driver_initialize(i2s_resources_t *resources, ARM_SAI_SignalEvent_t cb_event)
{
    ...
    init_conf.sync.autoTx           = true;
    init_conf.sync.autoCsEnable     = true;
    init_conf.sync.clockMode        = usartClockMode2;
    init_conf.format                = translate_conf_format(resources->_conf->databits, resources->_conf->word_size);
    init_conf.delay                 = true;
    init_conf.dmaSplit              = false;
    init_conf.justify               = resources->_conf->is_left_justified ? usartI2sJustifyLeft : usartI2sJustifyRight;
    init_conf.mono                  = resources->_conf->is_mono;

    /* config IOs */
    Driver_GPIO.Control(resources->_pins->i2s_rx, ARM_GPIO_SET_PIN_MODE, ARM_GPIO_PIN_MODE_INPUT_PULL);
    ...
    USART_InitI2s(resources->_peripheral, &init_conf);
    ...
    resources->__status.frame_error  = 0;
...
int32_t i2s_driver_control(i2s_resources_t *resources, uint32_t control, uint32_t arg, uint32_t another_arg)
{
    return ARM_DRIVER_ERROR_UNSUPPORTED;
}
...
{
    uint32_t flags   = USART_IntGetEnabled(resources->_peripheral);
    uint32_t status  = USART_StatusGet(resources->_peripheral);
    bool     is_left = (status & USART_STATUS_RXDATAVRIGHT) == 0;
    uint32_t event   = 0;
    ...
    if (flags & USART_IF_RXDATAV)
    {
        ...
        {
            USART_Rx(resources->_peripheral);
        }
        idx = (idx + 1) % 4;
    }
