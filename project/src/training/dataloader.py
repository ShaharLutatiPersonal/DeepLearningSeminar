...
class AudioDataset(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        for p in pair:
            sig,valid = read_audio(p,self.target_sr,self.T)
            if valid:
                audio.append(torch.tensor(sig))
            else:
                return 0,0,False
...
class DataLoader(object):
    ...
    def __init__(self, dataset = AudioDataset, batch_size = 10, drop_last=True):
        ...
        self.sampler = range(self.ds.__len__())
