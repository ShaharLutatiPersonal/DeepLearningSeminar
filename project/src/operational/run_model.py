...
def read_audio(fname, target_sr=8000, T=4, starting_point=0):
    ...
    if valid:
        data = data[starting_point*target_sr:starting_point *
...
def main_func(file_path, dict_path, starting_point):
    ...
    device = 'cpu'
    T = 4  # seconds
    model = mdl(64, 16, 256, 3, int(T*8e3), multi_loss=False,
                hidden_size=128, bidirectional=True, MulCat=True)
    model.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='path to audio file test.wav')
    parser.add_argument(
