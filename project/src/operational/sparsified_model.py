...
def tupple_p(blocknum, dict_weights):
    p1_w = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.P1.0.weight'.format(blocknum)])
    p1_b = dict_weights['seperation_net.blocklist.{}.P1.0.bias'.format(
        blocknum)]
    p2_w = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.P2.0.weight'.format(blocknum)])
    p2_b = dict_weights['seperation_net.blocklist.{}.P2.0.bias'.format(
        blocknum)]
    return (p1_w, p1_b, p2_w, p2_b)


def tupple_w_rnn(blocknum, number, dict_weights):
    wih = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.rnn_{}.weight_ih_l0'.format(blocknum, number)])
    whh = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.rnn_{}.weight_hh_l0'.format(blocknum, number)])
    bih = dict_weights['seperation_net.blocklist.{}.rnn_{}.bias_ih_l0'.format(
        blocknum, number)]
    bhh = dict_weights['seperation_net.blocklist.{}.rnn_{}.bias_hh_l0'.format(
        blocknum, number)]
    wih_rev = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.rnn_{}.weight_ih_l0_reverse'.format(blocknum, number)])
    whh_rev = to_sparse_encoded_tupple(
        dict_weights['seperation_net.blocklist.{}.rnn_{}.weight_hh_l0_reverse'.format(blocknum, number)])
    bih_rev = dict_weights['seperation_net.blocklist.{}.rnn_{}.bias_ih_l0_reverse'.format(
        blocknum, number)]
    bhh_rev = dict_weights['seperation_net.blocklist.{}.rnn_{}.bias_hh_l0_reverse'.format(
        blocknum, number)]
    wb_ih = (wih, bih)
    wb_hh = (whh, bhh)
...
def si_snr(x, y):
    ...
    ei_tild = y-si_tild
    si_snr = 10*torch.log10(torch.sum(si_tild*si_tild+eps) /
                            torch.sum(eps+ei_tild*ei_tild))
    return si_snr
...
class WolfModel(nn.Module):
    def __init__(self, N, L, K, num_blocks, T, multi_loss=False, hidden_size=64, bidirectional=True, MulCat=False, weights_for_seperation=[]):
        ...
        simple_net = nn.Conv1d(
            in_channels=1, out_channels=N, kernel_size=L, stride=L//2)
        ...
        self.encoding = nn.Conv1d(
            in_channels=1, out_channels=N, kernel_size=L, stride=L//2, bias=False)
        ...
        self.seperation_net = SeperationNet(in_channels=N, out_channels=N, hidden_channels=hidden_size, num_layer=num_blocks,
                                            num_speakers=2, bidirectional=self.bidirectional, MulCat=self.MulCat, w=weights_for_seperation)
        self.decoding = nn.Sequential(nn.PReLU(), nn.Conv2d(in_channels=R, out_channels=self.num_speakers*R,
                                                            kernel_size=1, bias=False), nn.BatchNorm2d(self.num_speakers*R, eps=self.eps), nn.ReLU())
        self.deconv = nn.Sequential(nn.ConvTranspose1d(
            in_channels=N, out_channels=1, kernel_size=L, stride=L//2, bias=False))
    ...
    def _over_add(self, input, gap):
        ...
        input1 = input[:, :, :, :K, :].contiguous().view(
            B, N, -1, C)[:, :, P:, :]
        input2 = input[:, :, :, K:, :].contiguous().view(
            B, N, -1, C)[:, :, :-P, :]
        input = input1 + input2
        # [B, N, L]
        if gap > 0:
    ...
    def forward(self, input):
        ...
        for i in range(1, self.num_blocks):
            if self.multi_loss:
                new_outputs.append(self.seperation_net.blocklist[i](
                    new_outputs[i-1]))  # BNKR
            else:
                new_outputs = [self.seperation_net.blocklist[i]
                               (new_outputs[0])]  # BNKR
        new_outputs = [self._over_add(self.decoding(x.permute(0, 3, 2, 1)).permute(
            0, 3, 2, 1).contiguous().view(B, N, K, R, self.num_speakers), gap) for x in new_outputs]
        deconv_outputs = []
...
class SeperationNet(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels, num_layer, num_speakers, bidirectional, MulCat, w):
        ...
        for i in range(num_layer):
            rnn_1_w, rnn_2_w, rnn_3_w, rnn_4_w, P1_w, P1_b, P2_w, P2_b = w[i]
            self.blocklist.append(SepBlock(out_channels, hidden_channels, num_speakers, bidirectional,
                                           MulCat, rnn_1_w, rnn_2_w, rnn_3_w, rnn_4_w, P1_w, P1_b, P2_w, P2_b))
...
class SepBlock(nn.Module):
    def __init__(self, out_channels, hidden_channels, num_speakers, bidirectional, MulCat, rnn_1_w, rnn_2_w, rnn_3_w, rnn_4_w, P1_w, P1_b, P2_w, P2_b):
        ...
        self.gn1 = nn.GroupNorm(
            num_groups=1, num_channels=out_channels, eps=1e-8, affine=True)
    ...
    def forward(self, input):
        ...
        if self.MulCat:
            ...
            short_term_fin1 = 1
        short_term_con = torch.cat(
            (short_term_fin*short_term_fin1, short_term), dim=2)
        transition_tensor = self.gn1(self.P1(short_term_con.contiguous().view(
            B*R*K, -1)).view(B*R, K, N).view(B, R, K, N).permute(0, 3, 2, 1).contiguous()) + input
        modified_transition = transition_tensor.permute(
            0, 2, 3, 1).contiguous().view(B*K, R, N)
        long_term1 = self.rnn_3(modified_transition)
        if self.MulCat:
            long_term2 = self.rnn_4(modified_transition)
        else:
            long_term2 = 1
        long_term_con = torch.cat(
            (long_term1*long_term2, modified_transition), dim=2)
        output = self.gn2(self.P2(long_term_con.contiguous().view(B*K*R, -1)).view(
            B*K, R, N).view(B, K, R, N).permute(0, 3, 1, 2).contiguous()) + transition_tensor
        return output
...
class LSTM_BI_DIR(nn.Module):
    def __init__(self, w, hidden_size, out_channels):
        ...
        self.lstm_forward = LSTM_Sparse(hidden_size, self.wih, self.bih, self.whh,
                                        self.bhh, out_channels, self.wih_rev, self.bih_rev, self.whh_rev, self.bhh_rev)
