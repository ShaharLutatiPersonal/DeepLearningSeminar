...
train_vec,train_vocab = import_ptb_dataset(dataset_type = 'train',path = './data',batch_size = batch_size)
print('loaded the training set, using the vocabulary for other sets')
valid_vec,dontcare = import_ptb_dataset(dataset_type = 'valid',path = './data',batch_size = batch_size,vocabulary = train_vocab)
test_vec,dontcare = import_ptb_dataset(dataset_type = 'test',path = './data',batch_size = batch_size,vocabulary = train_vocab)
print('done loading')
dropout_rate = [0.35,0]
...
def RunModel():
  for model_name in models_arr:
    for dp in dropout_rate:
        ...
        if model_name == 'lstm':
          model = RNN_Zam(dict_size = len(train_vocab),dp_prob = dp).to(device)
        else:
        ...
        patience = 4
        patience_iter = 2
        perpelxity_dict['{} {}'.format(model_name,dp)] = {'Train':[],'Validation':[],'Test':[]}
        for epoch in range(epoch_num):
            ...
            perpelxity_dict['{} {}'.format(model_name,dp)]['Train'].append(perplexity_loss(loss_term))
            perpelxity_dict['{} {}'.format(model_name,dp)]['Validation'].append(perplexity_loss(valid_error))
            perpelxity_dict['{} {}'.format(model_name,dp)]['Test'].append(perplexity_loss(test_error))
